...
- **Comma-separated coordinates**: `"chr1:1,000-2,000"`, `"chr1:100,000-200,000"`;
- **Space-delimited coordinates**: `"chr1 1000 2000"`;
- **Irregular spacing**: `"chr1  1000   2000"`, `"chr1: 1-10  |  chr2: 20-30"`;
- **Complex strings with unclear formatting**: `"chr1  1  10:+  |  chr2:20,000-30,000"`.
...
3. **Seamless integration**: Works with existing Bioconductor workflows through enhanced coercion methods;
...
Strings containing the pipe character (`|`) are automatically parsed as genomic interactions:
...
Sometimes you may want to force a specific object type, regardless of the automatic detection:
...
`GenomicCoordinates` seamlessly integrates with existing Bioconductor 
workflows by extending the standard coercion methods:

```{r}
# Enhanced GRanges coercion
as("chr1:100,000-200,000", "GRanges")

# Enhanced GPos coercion  
as("chr1:1,234,567", "GPos")

# Enhanced GInteractions coercion
as("chr1:1,000-10,000|chr2:20,000-30,000", "GInteractions")

# Enhanced IRanges coercion
as("1,000-2,000", "IRanges")
```

Of note, because it extends the `as` coercion method, the `GRanges()` function 
from `{GenomicRanges}` package will automatically use `GenomicCoordinates` for parsing:

```{r}
GRanges("chr1:100,000-200,000") ### This would fail without GenomicCoordinates loaded
```
